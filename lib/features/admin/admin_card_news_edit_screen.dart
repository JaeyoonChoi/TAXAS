...
class _AdminCardNewsEditScreenState
  ...
  Future<void> _save() async {
    ...
    final item = CardNewsItem(
      ...
      slides: _slides
          .map((s) => CardNewsSlide(
                heading: s.headingController.text.trim().isEmpty
                    ? null
                    : s.headingController.text.trim(),
                body: s.bodyController.text.trim().isEmpty
                    ? null
                    : s.bodyController.text.trim(),
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            _SectionLabel('색상 (이미지가 없을 때 사용)'),
            const SizedBox(height: 8),
            _GradientPicker(
              selected: _gradientIndex,
              onChanged: (i) => setState(() => _gradientIndex = i),
            ),
...
class _SlideDraft {
  ...
  _SlideDraft({String? heading, String? body, this.imageUrl})
      : headingController = TextEditingController(text: heading ?? ''),
        bodyController = TextEditingController(text: body ?? '');

  factory _SlideDraft.fromSlide(CardNewsSlide s) => _SlideDraft(
        heading: s.heading,
        body: s.body,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          TextFormField(
            controller: draft.headingController,
            decoration: const InputDecoration(
              labelText: '큰 제목 (heading)',
              hintText: '예: 자녀 공제 5억',
            ),
          ),
          const SizedBox(height: 10),
